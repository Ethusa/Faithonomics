...
const createCityNotNeutralEconomicUnitStep = (lessonId: string): Lesson["content"][number] => {
  const moralBudgetToolSrcDoc = "<!doctype html>\n<html lang=\"en-GB\">\n<head>\n<meta charset=\"utf-8\">\n<meta name=\"viewport\" content=\"width=device-width,initial-scale=1\">\n<title>Global Moral Budget Observatory</title>\n<script src=\"https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2\"></script>\n<style>\n:root{--paper:#F7F3E8;--cream:#E8D8B8;--kraft:#9A6B3F;--deep:#234638;--navy:#1E2A33;--gold:#B99245;--black:#17130F;--muted:#64685E;--line:#D8C8A8;--white:#FFFDF8}\n*{box-sizing:border-box}body{margin:0;padding:20px;font-family:Arial,sans-serif;color:var(--black);background:linear-gradient(#E9DDC7,#D7C3A2)}\n.app{width:min(1250px,100%);margin:auto;background:var(--paper);border:1px solid #ccb998;border-radius:22px;overflow:hidden;box-shadow:0 22px 58px rgba(0,0,0,.16)}\n.bar{height:5px;background:linear-gradient(90deg,var(--deep),var(--gold),var(--deep))}\n.hero,.section{padding:28px 34px}.section{border-top:1px solid var(--line)}.alt{background:#F1E7D5}\n.kicker{font-size:.78rem;text-transform:uppercase;letter-spacing:.08em;color:var(--kraft);font-weight:800}\nh1,h2,h3{font-family:Georgia,serif}h1{margin:.1em 0;color:var(--deep);font-size:clamp(2.3rem,4.7vw,4.6rem);line-height:.98}h2{color:var(--deep)}h3{color:var(--kraft)}\n.lead{color:var(--muted);line-height:1.62;max-width:1040px;text-align:justify;text-justify:inter-word}.search{display:grid;grid-template-columns:1fr auto;gap:10px;margin-top:16px}\ninput{padding:13px;border:1px solid var(--line);border-radius:10px;background:#fffdf8;font-size:1rem}.primary{padding:12px 16px;border-radius:10px;border:1px solid var(--gold);background:var(--deep);color:#f3dda7;font-weight:800;cursor:pointer}\n.status{display:none;margin-top:12px;padding:11px 13px;border-left:4px solid var(--gold);background:#faf4e8;color:#555d54}\n.results{display:none}.cityhead{display:flex;justify-content:space-between;gap:12px;align-items:flex-end;flex-wrap:wrap}.meta{font-size:.88rem;color:var(--muted)}\n.carousel{overflow:hidden;margin-top:14px}.track{display:flex;transition:transform .35s}.slide{min-width:100%;padding:2px}.card{padding:18px;border:1px solid var(--line);border-radius:17px;background:linear-gradient(145deg,#fffdf8,#efe5d5)}\n.cardtop{display:grid;grid-template-columns:50px 1fr auto;gap:12px;align-items:center}.icon{width:43px;height:43px;border-radius:50%;display:grid;place-items:center;background:var(--deep);color:#f2d99a;border:2px solid var(--gold);font-weight:900}.score{font-family:Georgia,serif;font-size:1.9rem;color:var(--deep);text-align:right}.score small{display:block;font-family:Arial,sans-serif;font-size:.72rem;color:var(--muted)}\n.evidence{margin-top:13px;padding:12px;border:1px solid var(--line);border-radius:11px;background:#fffdf8}.evidence ul{margin:.5em 0 0 1.1em;color:var(--muted);line-height:1.5;text-align:justify;text-justify:inter-word}.summary{margin-top:13px;color:#4e554c;line-height:1.58;text-align:justify;text-justify:inter-word}\n.nav{display:flex;justify-content:center;align-items:center;gap:9px;margin-top:12px}.nav button{width:38px;height:38px;border-radius:50%;border:1px solid var(--line);background:#fffdf8;cursor:pointer}.dots{display:flex;gap:6px}.dot{width:9px;height:9px;border-radius:50%;background:#cfc4ae}.dot.on{background:var(--gold)}\n.badge{display:inline-block;padding:6px 10px;border-radius:999px;background:var(--navy);color:#f3dda7;border:1px solid var(--gold);font-weight:800;font-size:.8rem}\n@media(max-width:650px){body{padding:0}.app{border-radius:0}.hero,.section{padding:22px 17px}.search{grid-template-columns:1fr}.cardtop{grid-template-columns:46px 1fr}.score{grid-column:1/-1;text-align:left}}\n</style></head>\n<body><main class=\"app\"><div class=\"bar\"></div>\n<section class=\"hero\"><div class=\"kicker\">Global Moral Budget Observatory</div><h1>What Does Your City Prioritise?</h1>\n<p class=\"lead\">Search the European Commission/JRC urban-centre database and explore <strong>Land, Time, Capital, Attention and Honour</strong>. Quantitative scores are shown only where evidence supports them.</p>\n<div class=\"search\"><input id=\"city\" type=\"search\" placeholder=\"Enter a city, e.g. Johannesburg\"><button id=\"go\" class=\"primary\">Search city</button></div><div id=\"status\" class=\"status\"></div></section>\n\n<section id=\"results\" class=\"section alt results\"><div class=\"cityhead\"><div><div class=\"kicker\">City profile</div><h2 id=\"name\"></h2><div id=\"meta\" class=\"meta\"></div></div><span id=\"sourceBadge\" class=\"badge\">GHS-UCDB R2024A</span></div>\n<div class=\"carousel\"><div id=\"track\" class=\"track\"></div></div><div class=\"nav\"><button id=\"prev\">&lsaquo;</button><div id=\"dots\" class=\"dots\"></div><button id=\"next\">&rsaquo;</button></div></section>\n<section class=\"section\"><div class=\"kicker\">Source and method</div><h2>European Commission/JRC Source Data</h2><p class=\"lead\">This observatory uses the European Commission Joint Research Centre GHS Urban Centre Database 2025, GHS-UCDB R2024A. The database contains 11,422 quality-controlled urban centres and is published under CC BY 4.0. Moral Budget profiles keep source measurements separate from interpretation: Land, Time and Capital may receive partial proxy scores where UCDB indicators support them, while Attention and Honour remain marked as needing local research unless appropriate local evidence is added.</p></section>\n</main>\n<script>\nconst CONFIG={SUPABASE_URL:\"https://mubisxvbvcdnrhdqnnyy.supabase.co\",SUPABASE_ANON_KEY:\"sb_publishable_7p_-KtzLqPDubr4ebv1zcA_9yvCb59u\"};\nconst dims=[[\"land\",\"LAND\",\"L\"],[\"time\",\"TIME\",\"T\"],[\"capital\",\"CAPITAL\",\"C\"],[\"attention\",\"ATTENTION\",\"A\"],[\"honour\",\"HONOUR\",\"H\"]];\nconst db=(!CONFIG.SUPABASE_URL.startsWith(\"YOUR_\"))?supabase.createClient(CONFIG.SUPABASE_URL,CONFIG.SUPABASE_ANON_KEY):null;\nlet idx=0;const $=id=>document.getElementById(id);\nconst normalise=s=>s.normalize(\"NFD\").replace(/[\\u0300-\\u036f]/g,\"\");\nfunction esc(s){return String(s??\"\").replace(/[&<>\"']/g,c=>({\"&\":\"&amp;\",\"<\":\"&lt;\",\">\":\"&gt;\",'\"':\"&quot;\",\"'\":\"&#039;\"}[c]))}\nfunction move(){track.style.transform=`translateX(-${idx*100}%)`;[...dots.children].forEach((d,i)=>d.classList.toggle(\"on\",i===idx))}\nfunction render(p){name.textContent=p.city_name;meta.textContent=[p.country,p.region_l1].filter(Boolean).join(\" - \");track.innerHTML=\"\";dots.innerHTML=\"\";\ndims.forEach(([k,t,l],i)=>{const score=p[k+\"_score\"],status=p[k+\"_status\"]||\"needs_local_research\",summary=p[k+\"_summary\"]||\"Assessment not yet available.\",ev=Array.isArray(p[k+\"_evidence\"])?p[k+\"_evidence\"]:[];\nconst s=document.createElement(\"div\");s.className=\"slide\";s.innerHTML=`<div class=\"card\"><div class=\"cardtop\"><div class=\"icon\">${l}</div><div><h3>${t}</h3><div class=\"meta\">${esc(status.replaceAll(\"_\",\" \"))}</div></div><div class=\"score\">${score==null?\"&mdash;\":Number(score).toFixed(1)+\"/5\"}<small>${score==null?\"no invented score\":\"comparative proxy score\"}</small></div></div><div class=\"summary\">${esc(summary)}</div><div class=\"evidence\"><strong>Evidence</strong>${ev.length?`<ul>${ev.slice(0,8).map(x=>`<li>${esc(x.indicator||x.label||JSON.stringify(x))}</li>`).join(\"\")}</ul>`:`<p class=\"meta\">Further local evidence required.</p>`}</div></div>`;track.appendChild(s);const d=document.createElement(\"span\");d.className=\"dot\"+(i===0?\" on\":\"\");dots.appendChild(d)});idx=0;move();results.style.display=\"block\";results.scrollIntoView({behavior:\"smooth\"})}\nasync function search(){const q=city.value.trim();if(!q)return;if(!db){status.style.display=\"block\";status.textContent=\"Add your Supabase URL and public anon key in CONFIG.\";return}status.style.display=\"block\";status.textContent=\"Searching...\";\nconst pattern=`%${q}%`,plain=`%${normalise(q)}%`;const {data,error}=await db.from(\"city_moral_budget_search\").select(\"*\").or(`city_name.ilike.${pattern},all_names.ilike.${pattern},search_text.ilike.${plain}`).order(\"population_latest\",{ascending:false,nullsFirst:false}).limit(5);const match=(data||[])[0];if(error){status.textContent=\"Database error. Check Supabase configuration.\";return}if(!match){status.textContent=\"No matching urban centre found.\";results.style.display=\"none\";return}status.style.display=\"none\";render(match)}\ngo.onclick=search;city.addEventListener(\"keydown\",e=>{if(e.key===\"Enter\")search()});prev.onclick=()=>{idx=(idx-1+dims.length)%dims.length;move()};next.onclick=()=>{idx=(idx+1)%dims.length;move()};\n</script></body></html>\n";
      ...
      <section class="city-neutral-page">
        ...
        <section class="moral-budget-panel">
          ...
          <iframe
            ...
            title="Global Moral Budget Observatory"
